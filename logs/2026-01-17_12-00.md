...
## Add log-rotate hook for file size warnings

User asked which hook enforces log file size limit — there wasn't one.

Created log-rotate.sh (afterFileEdit):
- Checks if edited file is a log file
- If > 4000 chars, appends warning to create new file
- Suggests new filename based on current timestamp

This makes log rotation hook-enforced instead of just a workflow rule.

## Auto-create new log file on rotation

User asked: why just warn? We should create the new file.

Updated log-rotate.sh to:
1. Create new log file with header (context, "continued from" reference)
2. Mark old log as rotated with pointer to new file
3. Update `logs/.active` with new `log_file` field

Added `log_file` field to .active format — tracks current active log file.
...
- Context: root
- Files touched: .cursor/rules/workflow.mdc,.cursor/commands/session.md,logs/2026-01-17_12-00.md,
- Timestamp: 2026-01-18 03:13

---

---
## ⚠️ LOG FILE SIZE LIMIT REACHED
This log file has exceeded ~4000 characters.
**Create a new log file:** `logs/2026-01-18_03-16.md`
---

---
## Log Rotated
Continued in: `logs/2026-01-18_03-18.md`
---
